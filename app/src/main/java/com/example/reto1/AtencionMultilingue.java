...
public class AtencionMultilingue extends AppCompatActivity {
    FirebaseFirestore db = FirebaseFirestore.getInstance();
    CollectionReference usersRef = db.collection("Citas");
    ObjetoCitas citaOL = new ObjetoCitas();
    String NombreCita= "Atencion Multilingue";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_atencion_multilingue);
        ...
        Button botonAtras=findViewById(R.id.atrasMulti); //  Para volver a la pantalla anterior
        ...
        botonMostrarFechas.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                citaOL.getArrayFechas().clear(); //Borra el array de fechas
                usersRef.addSnapshotListener(new EventListener<QuerySnapshot>() {
                    @Override
                    public void onEvent(@Nullable QuerySnapshot querySnapshot, @Nullable FirebaseFirestoreException error) {

                        for (QueryDocumentSnapshot document: querySnapshot){
                            if(document.getDate("fechaCitaAM1")!=null){ //Si la fecha no es null
                                // Log.d("TAG","OL fechas");
                                Date fechaCita = document.getDate("fechaCitaAM1");  //Recoge la fecha
                            ...
                            if(document.getDate("fechaCitaAM2")!=null){
                                Date fechaCita = document.getDate("fechaCitaAM2");
                            ...
                            if(document.getDate("fechaCitaAM3")!=null){
                                Date fechaCita = document.getDate("fechaCitaAM3");
                                citaOL.getArrayFechas().add(String.valueOf(fechaCita));

                                radioButtonB3.setVisibility(View.VISIBLE);
                                radioButtonB3.setText(String.valueOf(fechaCita));
                            }
                            if(document.getDate("fechaCitaAM4")!=null){
                                Date fechaCita = document.getDate("fechaCitaAM4");
                                citaOL.getArrayFechas().add(String.valueOf(fechaCita));

                                radioButtonB4.setVisibility(View.VISIBLE);
                                radioButtonB4.setText(String.valueOf(fechaCita));
                            }
                            if(document.getDate("fechaCitaAM5")!=null){
                                Date fechaCita = document.getDate("fechaCitaAM5");
                                citaOL.getArrayFechas().add(String.valueOf(fechaCita));
