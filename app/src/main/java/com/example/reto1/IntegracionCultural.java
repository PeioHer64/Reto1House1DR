...
public class IntegracionCultural extends AppCompatActivity {
    FirebaseFirestore db = FirebaseFirestore.getInstance();
    CollectionReference usersRef = db.collection("Citas");
    DocumentReference docuRef = usersRef.document("Cita");
    ObjetoCitas citaOL = new ObjetoCitas();
    String NombreCita= "Integracion Cultural";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_integracion_cultural);
        ...
        Button botonAtras=findViewById(R.id.atrasIntegracion); //  Para volver a la pantalla anterior
                ...
                usersRef.addSnapshotListener(new EventListener<QuerySnapshot>() {
                    ...
                    public void onEvent(@Nullable QuerySnapshot querySnapshot, @Nullable FirebaseFirestoreException error) {
                        ...
                        for (QueryDocumentSnapshot document: querySnapshot){
                            if(document.getDate("fechaCitaIC1")!=null){ //Si la fecha no es null
                                // Log.d("TAG","OL fechas");
                                Date fechaCita = document.getDate("fechaCitaIC1");  //Recoge la fecha
                            ...
                            if(document.getDate("fechaCitaIC2")!=null){
                                Date fechaCita = document.getDate("fechaCitaIC2");
                            ...
                            if(document.getDate("fechaCitaIC3")!=null){
                                Date fechaCita = document.getDate("fechaCitaIC3");
                                citaOL.getArrayFechas().add(String.valueOf(fechaCita));

                                radioButtonB3.setVisibility(View.VISIBLE);
                                radioButtonB3.setText(String.valueOf(fechaCita));
                            }
                            if(document.getDate("fechaCitaIC4")!=null){
                                Date fechaCita = document.getDate("fechaCitaIC4");
                                citaOL.getArrayFechas().add(String.valueOf(fechaCita));

                                radioButtonB4.setVisibility(View.VISIBLE);
                                radioButtonB4.setText(String.valueOf(fechaCita));
                            }
                            if(document.getDate("fechaCitaIC5")!=null){
                                Date fechaCita = document.getDate("fechaCitaIC5");
                                citaOL.getArrayFechas().add(String.valueOf(fechaCita));
